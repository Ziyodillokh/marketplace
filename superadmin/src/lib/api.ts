...
export function setAccessToken(token: string | null): void {
  ...
  if (typeof window !== 'undefined') {
    if (token) localStorage.setItem('super_at', token);
    else localStorage.removeItem('super_at');
  }
}
...
export function loadAccessToken(): string | null {
  ...
  accessToken = localStorage.getItem('super_at');
  return accessToken;
}

function buildUrl(path: string, query?: Record<string, unknown>): string {
  const base =
    typeof window !== 'undefined' ? window.location.origin + API_URL : `http://localhost:5180${API_URL}`;
...
async function refreshToken(): Promise<string> {
  ...
  refreshPromise = (async () => {
    const res = await fetch(buildUrl('/super-admin/auth/refresh'), {
      method: 'POST',
      credentials: 'include',
    });
    if (!res.ok) {
